...
while($row = mysql_fetch_array($result)){
	array_push($arr_data, $row['batt']);
	array_push($arr_xticks, $row['uts']);
}

// Create the graph. These two calls are always required
$graph = new Graph(800,600,"auto");	     // SCALE
$graph->SetScale("datint",3.5,5.5);            // SET THE SCALE MANUALLY
...
$graph->title->Set("System Voltage - " . date("G:i, F j", $uts_onedayago) . " to " . date("G:i, F j", $uts_now));
$graph->xaxis->SetTitle('Time','middle'); 
$graph->yaxis->SetTitle('Voltage / V','middle');
