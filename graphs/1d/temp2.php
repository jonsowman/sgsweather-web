...
while($row = mysql_fetch_array($result)){
	array_push($arr_data, $row['temp']);
	array_push($t2_data, $row['temp2']);
	array_push($arr_xticks, $row['uts']);
}

// Create the graph. These two calls are always required
$graph = new Graph(800,600,"auto");	     // SCALE
$graph->SetScale("datint",-10,40);            // SET THE SCALE MANUALLY
$graph->SetY2Scale("lin",-10,40);            // SET THE SCALE MANUALLY
...
$graph->title->Set("Temperature - " . date("G:i, F j", $uts_onedayago) . " to " . date("G:i, F j", $uts_now));
...
$lineplot2=new LinePlot($t2_data, $arr_xticks);
...
$lineplot->SetLegend ("Exp. Temp");
$lineplot2->SetLegend("Shl. Temp");
