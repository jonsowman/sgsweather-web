...
include ("../../includes/jpgraph/jpgraph_radar.php");
...
while($row = mysql_fetch_array($result)){
	array_push($arr_data, $row['wind_dir']);
	//array_push($arr_xticks, $row['uts']);
}

// Create the graph. These two calls are always required
$graph = new RadarGraph(800,600,"auto");	     // SCALE
//$//graph->SetScale("datint",0,360);            // SET THE SCALE MANUALLY
...
//$graph->title->Set("Wind Direction - " . date("G:i, F j", $uts_onedayago) . " to " . date("G:i, F j", $uts_now));
//$graph->xaxis->SetTitle('Time','middle'); 
//$graph->yaxis->SetTitle('Wind Direction / Degrees','middle');
//$graph->xaxis->SetTitleMargin('50'); 

$plot=new RadarPlot($arr_data);       // Y-DATA, X-DATA (TIME)
                 // SET LINE THICKNESS
