...
$WndS_MA_N = $_GET['ma'];
if ($WndS_MA_N == 0){$WndS_MA_N = 15;}

$t = 0;

while($row = mysql_fetch_array($result)){
	array_push($arr_data, $row['wind_spd']);
	...
	IF ($WndS_MA_Ptr >= $WndS_MA_N){
		...
		array_push($WndS_MA_data, $t/$WndS_MA_N);
		$t = 0;
	}else{	
		array_push($WndS_MA_temp, $row[wind_spd]);
...
$graph->SetScale("datint");            // SET THE SCALE MANUALLY
...
$graph->title->Set("Wind Speed - " . date("G:i, F j", $uts_onedayago) . " to " . date("G:i, F j", $uts_now));
...
$lineplot2=new LinePlot($WndS_MA_data, $arr_xticks);       // Y-DATA, X-DATA (TIME)
...
// PLOT OPTIONS2
$lineplot2->SetColor("black");                // LINE COLOUR
if ($_GET['xm'] != '0'){$lineplot2->mark->SetType(MARK_X, 0.3);}
$lineplot2->mark->SetColor("black");          // POINTS COLOUR
$lineplot2->mark->Hide();                   // SET THEM TO DISPLAY
